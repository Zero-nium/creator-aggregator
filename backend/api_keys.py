...
def _ensure_table():
    """Create api_keys table in the same DB as signals. Handles Turso stream errors."""
    db._ensure_init()
    try:
        if db._client:
            db._client.execute("""
        ...
        # If table already exists, ignore. If Hrana stream error, re-init and retry once.
        if "already exists" in str(e).lower():
            return
        # Try re-init DB connection and retry
        print(f"[api_keys] Table creation failed, retrying: {e}")
...
def verify_key(api_key: str):
    ...
    try:
        ...
        # Hrana stream error — re-init and retry once
        print(f"[api_keys] Verify failed, retrying: {e}")
        db._initialized = False
        db._client = None
        db._local_conn = None
        db._ensure_init()
